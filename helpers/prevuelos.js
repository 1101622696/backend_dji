...
const getPrevuelosConEtapasEmail = async (email) => {
  try {
    const prevuelosconetapasemail = await getPrevuelosConEtapas();
 
        // Filtrar por el email del usuario
        return prevuelosconetapasemail.filter(prevuelo => 
          prevuelo.useremail && prevuelo.useremail.toLowerCase() === email.toLowerCase()
        );
      } catch (error) {
        console.error('Error al obtener prevuelos con etapas por email:', error);
        throw error;
      }
